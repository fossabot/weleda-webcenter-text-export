#!/bin/bash
set -e -u -o pipefail
# https://github.com/nginxinc/docker-nginx/blob/master/stable/debian/30-tune-worker-processes.sh
...
touch /etc/nginx/nginx.conf 2>/dev/null || { entrypoint_error "can not modify /etc/nginx/nginx.conf (read-only file system?)"; exit 0; }
